...
for i in range(1,665):
  ...
  for link in soup2.find_all('a', attrs={'class': 'color-charcoal course-name'}):
      ...
      Image = link_soup2.find('img',attrs={'class':'absolute'})
      if Image:
          Image = Image.get('src').strip()
      else:
          Image = ""
      title = link_soup2.find('h1',attrs={'class':'head-2'})
      if title:
          title = title.get_text().strip()
      else:
          title = ""
      Overview = link_soup2.find('div',attrs={'class':'wysiwyg'})
      if Overview:
          Overview = Overview.get_text().strip()
      else:
          Overview = ""
      taught_by = link_soup2.find('p',attrs={'class':'text-1'})
      if taught_by:
          taught_by = taught_by.get_text().strip()
      else:
          taught_by = ""
